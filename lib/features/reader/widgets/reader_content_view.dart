...
class _ReaderContentViewState extends State<ReaderContentView> {
  ...
  void didUpdateWidget(ReaderContentView oldWidget) {
    ...
    if (oldWidget.paged != widget.paged ||
        ...
        oldWidget.centeredText != widget.centeredText) {
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) => Align(
          ...
          child: SizedBox(
            width: _columnWidth(_viewport),
            child: slot.rendered[index],
          ),
        ),
      ),
    );
  ...
  double _columnWidth(Size viewport) {
    ...
    // 双页本身已经把可读宽度分成两栏，不再二次收窄。
    if (!widget.centeredText || _columns > 1) return width;
    return math.min(width, widget.chapter.style.fontSize * 32);
  }
  ...
  Widget _pageColumn(int globalColumn, Size viewport, int column, int columns) {
    ...
    return ReaderPageBody(
      ...
      viewport: viewport,
      padding: padding,
      contentWidth: _columnWidth(viewport),
    );
  }
